    ...
    async function fetchData() {
      try {
        ...
        const resPostulaciones = await fetch(
          `http://backenddevconnect.work.gd:8000/api/postulacions?ofertalaboral=/api/ofertalaborals/${id}`, 
        ...
        const resOferta = await fetch(
          `http://backenddevconnect.work.gd:8000/api/ofertalaborals/${id}`, 
          { headers: { Accept: 'application/ld+json' } }
        );
        ...
        const usuariosFetches = postulacionesList.map(postulacion => {
          ...
          return fetch(`http://backenddevconnect.work.gd:8000/api/usuarios/${usuarioId}`, {
            headers: { Accept: 'application/ld+json' }
          }).then(res => {
    ...
    <div className="container py-5 candidates-page">
      ...
      <ul className="list-unstyled candidates-page__list">
        {postulaciones.map((postulacion) => {
          ...
          const cvUrl = `http://backenddevconnect.work.gd:8000/descargar-cv/${id}/${usuarioId}`;
